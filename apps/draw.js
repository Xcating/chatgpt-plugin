...
export class dalle extends plugin {
  ...
  async draw(e) {
    ...
    await this.reply(
      `正在为您绘制大小为${size}的${num}张图片，预计消耗${
        priceMap[size] * num
      }美元余额，请稍候……`
    );
    try {
      let images = (await createImage(prompt, num, size)).map((image) =>
  ...
  async variation(e) {
    ...
    try {
      let images = (await imageVariation(imgUrl)).map((image) =>
        segment.image(`base64://${image}`)
      );
      if (images.length > 1) {
  ...
  async edit(e) {
    ...
    let [prompt = "", position = "", num = "1", size = "512x512"] = args.slice(
      0,
      4
    );
    if (!prompt || !position) {
      this.reply(
        "编辑图片必须填写prompt和涂抹位置.参考格式：A bird on it/100,100,300,200/2/512x512"
      );
      return false;
    }
    num = parseInt(num, 10);
    ...
    try {
      let images = (
        await editImage(
          imgUrl,
          position.split(",").map((p) => parseInt(p, 10)),
          prompt,
          num,
          size
        )
  ...
  async bingDraw(e) {
    ...
    if ((await redis.exists("CHATGPT:BING_TOKENS")) != 0) {
      ...
      const restricted = bingTokens.filter(
        (element) => element.State === "受限"
      );
      if (normal.length > 0) {
    ...
    if (!bingToken) {
      throw new Error(
        "未绑定Bing Cookie，请使用#chatgpt设置必应token命令绑定Bing Cookie"
      );
    }
    // 记录token使用
    let bingTokens = JSON.parse(await redis.get("CHATGPT:BING_TOKENS"));
