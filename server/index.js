          ...
          switch (data.command) {
            case "register":
              if (data.state) {
                ...
                Bot.sendPrivateMsg(
                  master,
                  `当前chatgpt插件服务无法被外网访问，已启用代理链接，访问代码：${data.token}`,
                  false
                );
              } else {
                console.log("注册区域失败");
            ...
            case "post_login":
              if (data.qq && data.passwd) {
                ...
                if (
                  data.qq == Bot.uin &&
                  (await redis.get("CHATGPT:ADMIN_PASSWD")) == data.passwd
                ) {
                  ...
                  if (user.passwd != "" && user.passwd === data.passwd) {
                    ...
                    ws.send(
                      JSON.stringify({
                        command: data.command,
                        state: false,
                        error: `用户名密码错误,如果忘记密码请私聊机器人输入 ${
                          data.qq == Bot.uin ? "#修改管理密码" : "#修改用户密码"
                        } 进行修改`,
                        region: Bot.uin,
                        type: "server",
                      })
            ...
            case "post_command":
              ...
              const response = await fetch(
                `http://localhost:${Config.serverPort || 3321}${data.postPath}`,
                fetchOptions
              );
              if (response.ok) {
...
export async function createServer() {
    ...
    if (body.content) {
      ...
      try {
        ...
        const data = {
          ...
          herf: `http://${
            body.cacheHost || ip + ":" + Config.serverPort || 3321
          }/page/${body.entry}`,
        ...
        reply.send({
          file: body.entry,
          cacheUrl: `http://${ip}:${Config.serverPort || 3321}/page/${
            body.entry
          }`,
        });
      } catch (err) {
  ...
  server.post("/cleanCache", async (request, reply) => {
    ...
    userData.chat.forEach(function (item, index) {
      const filename =
        item.herf.substring(item.herf.lastIndexOf("/") + 1) + ".json";
      const filepath = path.join(dir, filename);
        ...
        switch (data.command) {
          case "sendMsg": // 代理消息发送
            ...
            if (data.id && data.message) {
              ...
              await connection.socket.send(
                JSON.stringify({ command: data.command, state: true })
              );
            } else {
              await connection.socket.send(
                JSON.stringify({
            ...
            if (!connection.login) {
              ...
              await connection.socket.send(
                JSON.stringify({
                  command: data.command,
                  state: true,
                  user: { user: user.user, autho: user.autho },
                })
              );
            }
            break;
  ...
  server.post("/sysconfig", async (request, reply) => {
    ...
    if (!user) {
      ...
      if ((await redis.exists("CHATGPT:?")) != 0) {
        redisConfig.openAiPlatformAccessToken = await redis.get(
          "CHATGPT:TOKEN"
        );
      }
      reply.send({
  ...
  server.post("/saveconfig", async (request, reply) => {
    ...
    if (!user) {
      ...
      if (body.userSetting) {
        await redis.set(
          `CHATGPT:USER:${user.user}`,
          JSON.stringify(body.userSetting)
        );
      }
      if (body.userConfig) {
  ...
  server.post("/serverTest", async (request, reply) => {
    ...
    if (Config.cloudTranscode) {
      const checkCheckCloud = await fetch(Config.cloudTranscode, {
        method: "GET",
      });
  ...
  let job_Statistics_SystemLoad = schedule.scheduleJob(
    "0 * * * *",
    async function () {
      Statistics.SystemLoad.count = await getLoad();
