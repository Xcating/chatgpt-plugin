...
export class help extends plugin {
  ...
  async detailPrompt(e) {
    ...
    await e.reply(
      `《${prompt.name}》\n${limitString(prompt.content, 500)}`,
      true
    );
  }
  ...
  async usePrompt(e) {
    ...
    if (keyMap[use]) {
      ...
      await e.reply(
        `你当前正在使用${use}模式，已将该模式设定应用为"${promptName}"。更该设定后建议结束对话以使设定更好生效`,
        true
      );
    } else {
      if (use == "bing") {
        await e.reply(
          `你当前正在使用${use}模式，该模式不支持设定。请使用 #chatgpt必应切换自设定 切换到自设定模式使用设定`,
          true
        );
      } else {
        await e.reply(
          `你当前正在使用${use}模式，该模式不支持设定。支持设定的模式有：API、自定义、Claude`,
          true
        );
      }
    }
  ...
  async uploadPrompt(e) {
    ...
    let currentUse = e.msg.replace(
      /^#(chatgpt|ChatGPT)(上传|分享|共享)设定/,
      ""
    );
    if (!currentUse) {
    ...
    await this.reply(
      `即将向云端上传设定${currentUse}，确定请回复确定，取消请回复取消，或者回复其他本地存在设定的名字`,
      true
    );
    let extraData = {
      currentUse,
  ...
  async uploadPromptR18() {
    ...
    if (response.status === 200) {
      ...
      if (response.data === true) {
        await this.reply(
          `设定${currentUse}已上传，其他人可以通过#chatgpt导入设定${currentUse} 来快速导入该设定。感谢您的分享。`,
          true
        );
      } else {
  ...
  async detailCloudPrompt(e) {
    ...
    let response = await fetch(
      "https://chatgpt.roki.best/prompt?name=" + name,
      {
        method: "GET",
        headers: {
          "FROM-CHATGPT": "ikechan8370",
  ...
  async browsePrompt(e) {
    ...
    let response = await fetch(
      "https://chatgpt.roki.best/prompt/list?search=" +
        search +
        `&page=${page - 1}`,
      {
        method: "GET",
    ...
    if (response.status === 200) {
      ...
      output +=
        "----------------------------------------------------------------------------------------\n";
      content.forEach((c) => {
        output += `|  【${c.title}】  |  ${maskQQ(c.qq)}  | ${c.createTime} | ${
          c.r18
        } | ${c.use}｜\n`;
      });
      output +=
      ...
      }页，共${totalElements}个设定\n`;
      output += `                       您可以使用#chatgpt浏览设定页码${
        pageable.pageNumber + 2
      }跳转到第${pageable.pageNumber + 2}页\n`;
  ...
  async importPrompt(e) {
    ...
    let response = await fetch(
      "https://chatgpt.roki.best/prompt?name=" + promptName,
      {
        method: "GET",
        headers: {
          "FROM-CHATGPT": "ikechan8370",
    ...
    if (response.status === 200) {
      ...
      if (r.code === 200) {
        ...
        e.reply(
          `导入成功。您现在可以使用 #chatgpt使用设定${title} 来体验这个设定了。`
        );
      } else {
  ...
  async helpPrompt() {
    await this.reply(
      "设定目录为/plugins/chatgpt-plugin/prompts，将会读取该目录下的所有[设定名].txt文件作为设定列表",
      true
    );
  }
}
