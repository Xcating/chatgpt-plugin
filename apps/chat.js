...
import {
  deleteConversation,
  getConversations,
  getLatestMessageIdByConversationId,
...
try {
  ...
  logger.info(
    ...
    logger.yellow(`[依赖管理]`),
    logger.red(`[缺少依赖]`),
    "[ChatGPT-Plugin]依赖emoji-strip未安装，会导致azure语音模式下朗读emoji的问题，建议执行pnpm install emoji-strip安装"
  );
}
try {
  await import("keyv");
  ...
  logger.info(
    ...
    logger.yellow(`[依赖管理]`),
    logger.red(`[缺少依赖]`),
    "[ChatGPT-Plugin]依赖keyv未安装，可能影响Sydney模式下Bing对话，建议执行pnpm install keyv安装"
  );
}
...
const defaultPropmtPrefix =
  ", a large language model trained by OpenAI. You answer as concisely as possible for each response (e.g. don’t be verbose). It is very important that you answer as concisely as possible, so please remember this. If you are generating a list, do not have too many items. Keep the number of items short.";
...
export class chatgpt extends plugin {
  ...
  async getConversations(e) {
    ...
    if (!keys || keys.length === 0) {
      ...
      let response =
        "当前对话列表：(格式为【开始时间 ｜ qq昵称 ｜ 对话长度 ｜ 最后活跃时间】)\n";
      await Promise.all(
        keys.map(async (key) => {
    ...
    if (ats.length === 0) {
      if (use === "api3") {
        ...
        logger.info(
          ...
          logger.yellow(`[聊天]`),
          logger.red(`[Sydney]`),
          `SydneyUser_${e.sender.user_id}`,
          await conversationsCache.get(`SydneyUser_${e.sender.user_id}`)
        );
        ...
        logger.info(
          ...
          logger.yellow(`[聊天]`),
          logger.red(`[ChatGML]`),
          `ChatGLMUser_${e.sender.user_id}`,
          await conversationsCache.get(`ChatGLMUser_${e.sender.user_id}`)
        );
        ...
        let c = await redis.get(
          `CHATGPT:CONVERSATIONS_BROWSER:${e.sender.user_id}`
        );
        if (!c) {
          await this.reply("当前没有开启对话", true);
        } else {
        ...
        if (!cookie) {
          throw new Error(
            "未绑定Poe Cookie，请使用#chatgpt设置Poe token命令绑定cookie"
          );
        }
      ...
      if (use === "api3") {
        ...
        await this.reply(
          `${atUser}已退出TA当前的对话，TA仍可以@我进行聊天以开启新的对话`,
          true
        );
      } else if (
        use === "bing" &&
        ...
        for (let i = 0; i < cs.length; i++) {
          ...
          if (Config.debug) {
            logger.info(
              ...
              logger.yellow(`[聊天]`),
              logger.red(`[必应结束所有对话]`),
              "delete bing conversation of qq: " + cs[i]
            );
          }
          deleted++;
        }
        ...
        for (let i = 0; i < cs.length; i++) {
          ...
          if (Config.debug) {
            logger.info(
              logger.cyan("[ChatGPT-plugin]"),
              logger.yellow(`[聊天]`),
              logger.red(`[API结束所有对话]`),
              "delete api conversation of qq: " + cs[i]
            );
          }
          deleted++;
        }
        ...
        for (let i = 0; i < qcs.length; i++) {
          ...
          if (Config.debug) {
            logger.info(
              logger.cyan("[ChatGPT-plugin]"),
              logger.yellow(`[聊天]`),
              logger.red(`[API3结束所有对话]`),
              "delete conversation bind: " + qcs[i]
            );
          }
          deleted++;
        }
        ...
        for (let i = 0; i < qcs.length; i++) {
          ...
          if (Config.debug) {
            logger.info(
              logger.cyan("[ChatGPT-plugin]"),
              logger.yellow(`[聊天]`),
              logger.red(`[GML结束所有对话]`),
              "delete chatglm conversation bind: " + qcs[i]
            );
          }
          deleted++;
        }
  ...
  async deleteConversation(e) {
    ...
    if (ats.length === 0 || (ats.length === 1 && (e.atme || e.atBot))) {
      ...
      if (!conversationId) {
        await this.reply(
          "指令格式错误，请同时加上对话id或@某人以删除他当前进行的对话",
          true
        );
        return false;
      } else {
        ...
        await this.reply(
          `对话删除成功，同时清理了${deleted}个同一对话中用户的对话。`,
          true
        );
      }
    } else {
      for (let u = 0; u < ats.length; u++) {
        ...
        if (conversationId) {
          ...
          await this.reply(
            `${atUser}的对话${conversationId}删除成功，同时清理了${deleted}个同一对话中用户的对话。`
          );
        } else {
  ...
  async switch2Picture(e) {
    ...
    await redis.set(
      ...
      JSON.stringify(userReplySetting)
    );
  ...
  async switchTTSSource(e) {
    ...
    switch (target.trim()) {
      ...
      default: {
        await e.reply(
          "请使用#chatgpt语音换源+数字进行换源。1为vits-uma-genshin-honkai，2为微软Azure，3为voicevox"
        );
        return;
      }
    }
  ...
  async setDefaultRole(e) {
    if (Config.ttsMode === "vits-uma-genshin-honkai" && !Config.ttsSpace) {
      await this.reply(
        "您没有配置vits-uma-genshin-honkai API，请前往后台管理或锅巴面板进行配置"
      );
      return;
    }
    if (Config.ttsMode === "azure" && !Config.azureTTSKey) {
      await this.reply(
        "您没有配置azure 密钥，请前往后台管理或锅巴面板进行配置"
      );
      return;
    }
    if (Config.ttsMode === "voicevox" && !Config.voicevoxSpace) {
      await this.reply(
        "您没有配置voicevox API，请前往后台管理或锅巴面板进行配置"
      );
      return;
    }
    ...
    switch (Config.ttsMode) {
      case "vits-uma-genshin-honkai": {
        ...
        if (speakers.indexOf(userSetting.ttsRole) >= 0) {
          ...
          await this.reply(
            `当前语音模式为${Config.ttsMode},您的默认语音角色已被设置为 "${userSetting.ttsRole}" `
          );
        } else if (speaker === "随机") {
          ...
          await this.reply(
            `当前语音模式为${Config.ttsMode},您的默认语音角色已被设置为 "随机" `
          );
        } else {
      ...
      case "azure": {
        let userSetting = await getUserReplySetting(this.e);
        let chosen = AzureTTS.supportConfigurations.filter(
          (s) => s.name === speaker
        );
        if (speaker === "随机") {
          ...
          await this.reply(
            `抱歉，没有"${speaker}"这个角色，目前azure模式下支持的角色有${AzureTTS.supportConfigurations
              .map((item) => item.name)
              .join("、")}`
          );
        } else {
          ...
          const supportEmotion = AzureTTS.supportConfigurations.find(
            (config) => config.name === speaker
          )?.emotion;
          await this.reply(
            `当前语音模式为${
              Config.ttsMode
            },您的默认语音角色已被设置为 ${speaker}-${chosen[0].gender}-${
              chosen[0].languageDetail
            } ${
              supportEmotion && Config.azureTTSEmotion
      ...
      case "voicevox": {
        ...
        await this.reply(
          `当前语音模式为${Config.ttsMode},您的默认语音角色已被设置为 "${userSetting.ttsRoleVoiceVox}" `
        );
        break;
      }
    }
  ...
  async abstractChat(e, prompt, use) {
    ...
    if (whitelist.join("").length > 0) {
      for (const item of whitelist) {
        if (item.length > 11) {
          ...
          item.startsWith("^") &&
          item.slice(1) === e.sender.user_id.toString()
        ) {
          chatPermission = true;
          break;
        } else if (
    ...
    if (
      trySplit.length > 1 &&
      speakers.indexOf(convertSpeaker(trySplit[0])) > -1
    ) {
      useTTS = true;
    ...
    // 检索是否有屏蔽词
    const promtBlockWord = Config.promptBlockWords.find((word) =>
          ...
          if ((await redis.lIndex("CHATGPT:CHAT_QUEUE", 0)) === randomId) {
            ...
            if ((await redis.exists("CHATGPT:CHAT_QUEUE_TIMEOUT")) === 0) {
              ...
              await redis.pSetEx(
                "CHATGPT:CHAT_QUEUE_TIMEOUT",
                Config.defaultTimeoutMs,
                await redis.lIndex("CHATGPT:CHAT_QUEUE", 0)
              );
              if (confirmOn) {
                let length = (await redis.lLen("CHATGPT:CHAT_QUEUE")) - 1;
                await this.reply(
                  `问题想不明白放弃了，开始思考下一个问题，当前队列前方还有${length}个问题`,
                  true,
                  { recallMsg: 8 }
                );
                logger.info(
                  ...
                  logger.yellow(`[聊天]`),
                  logger.red(`[API3排队]`),
                  `问题超时已弹出，chatgpt队列前方还有${length}个问题。管理员可通过#清空队列来强制清除所有等待的问题。`
                );
              }
            }
            await delay(1500);
    ...
    const emotionFlag = await redis.get(
      `CHATGPT:WRONG_EMOTION:${e.sender.user_id}`
    );
    let userReplySetting = await getUserReplySetting(this.e);
    // 图片模式就不管了，降低抱歉概率
    if (
      Config.ttsMode === "azure" &&
      Config.enhanceAzureTTSEmotion &&
      userReplySetting.useTTS === true &&
      (await AzureTTS.getEmotionPrompt(e))
    ) {
      switch (emotionFlag) {
        case "1":
          prompt +=
            "(上一次回复没有添加情绪，请确保接下来的对话正确使用情绪和情绪格式，回复时忽略此内容。)";
          break;
        case "2":
          prompt +=
            "(不要使用给出情绪范围的词和错误的情绪格式，请确保接下来的对话正确选择情绪，回复时忽略此内容。)";
          break;
        case "3":
          prompt +=
            "(不要给出多个情绪[]项，请确保接下来的对话给且只给出一个正确情绪项，回复时忽略此内容。)";
          break;
      }
    }
    logger.info(
    ...
    if (use === "api3") {
      ...
      if (conversationId) {
        ...
        if (!lastMessageId) {
          lastMessageId = await getLatestMessageIdByConversationId(
            conversationId,
            newFetch
          );
        }
    ...
    try {
      ...
      if (use !== "api3" && use !== "poe" && use !== "claude") {
        ...
        if (use === "bing" && !chatMessage.error) {
          ...
          previousConversation.conversationSignature =
            chatMessage.conversationSignature;
      ...
      if (
        ...
        const emotionTimes = response.match(
          /\[\s*['`’‘]?(\w+)[`’‘']?\s*[,，、]\s*([\d.]+)\s*\]/g
        );
      ...
      // 检索是否有屏蔽词
      const blockWord = Config.blockWords.find((word) =>
      ...
      const shouldAddClosingBlock =
        codeBlockCount % 2 === 1 && !response.endsWith("```");
      ...
      if (responseUrls) {
        let images = await Promise.all(
          responseUrls.map((link) => isImage(link))
        );
      ...
      if (useTTS) {
        ...
        try {
          ...
          await this.reply(
            "依赖emoji-strip未安装，请执行pnpm install emoji-strip安装依赖",
            true
          );
        }
        ...
        if (
          Config.alsoSendText ||
          ttsResponse.length > parseInt(Config.ttsAutoFallbackThreshold)
        ) {
          if (
            Config.ttsMode === "vits-uma-genshin-honkai" &&
            ttsResponse.length > parseInt(Config.ttsAutoFallbackThreshold)
          ...
          await this.reply(
            await convertFaces(response, Config.enableRobotAt, e),
            e.isGroup
          );
          if (quotemessage.length > 0) {
            this.reply(
              await makeForwardMsg(
                this.e,
                quotemessage.map((msg) => `${msg.text} - ${msg.url}`)
              )
            );
          }
          if (
        ...
        const sendable = await generateAudio(
          this.e,
          ttsResponse,
          emotion,
          emotionDegree
        ...
        userSetting.usePicture ||
        (Config.autoUsePicture &&
          response.length > Config.autoUsePictureThreshold)
      ) {
        ...
        try {
          ...
          logger.warn(
            "error happened while uploading content to the cache server. QR Code will not be showed in this picture."
          );
          logger.error(err);
      ...
      if (use == "api") {
        if (
          msg ==
          'Error: ChatGPT error 404: {"detail":"Unknown model! Follow list from /api/models!"}'
        ) {
          await this.reply(
            `ChatGPT通信异常,当前模型不存在或不可用，请尝试发送#chatgpt模型大全，并更换其他模型`,
            true
          );
          ...
          msg.includes(
            'Error: ChatGPT error 400: {"detail":"Status code is 500 from reverse engineered site"}'
          )
        ) {
          ...
          await this.reply(
            `未设置OpenAI密钥，如果你配置了BingToken还提示这个，请使用 #chatgpt切换Bing 切换到必应模式`,
            true
          );
        } else if (msg.includes("TimeoutError")) {
          await this.reply(
            `OpenAI ChatGPT 访问超时，请检查代理或者超时时间设置`,
            true
          );
        } else {
          if (msg.length < 200) {
      ...
      if (use == "api3") {
        if (msg.includes('"statusCode":401')) {
          ...
          await this.reply(
            `你的API3 Token对应的账号被OpenAI封禁，请更换`,
            true
          );
  ...
  async cacheContent(
    ...
    const cacheres = await fetch(
      Config.viewHost
        ? `${Config.viewHost}/`
        : `http://127.0.0.1:${Config.serverPort || 3321}/` + "cache",
      cacheresOption
    );
    if (cacheres.ok) {
  ...
  async renderImage(
    ...
    let cacheData = await this.cacheContent(
      e,
      use,
      content,
      prompt,
    ...
    const template =
      use !== "bing" ? "content/ChatGPT/index" : "content/Bing/index";
    if (!Config.oldview) {
      if (cacheData.error || cacheData.status != 200) {
        await this.reply(
          `出现错误：${cacheData.error || "server error " + cacheData.status}`,
          true
        );
      } else {
        await e.reply(
          await renderUrl(
            ...
            (Config.viewHost
              ? `${Config.viewHost}/`
              : `http://127.0.0.1:${Config.serverPort || 3321}/`) +
              `page/${cacheData.file}?qr=${
                Config.showQRCode ? "true" : "false"
              }`,
            {
              ...
              func:
                parseFloat(Config.live2d) && !Config.viewHost
                  ? "window.Live2d == true"
                  : "",
  ...
  async sendMessage(prompt, conversation = {}, use, e) {
    ...
    switch (use) {
      ...
      case "bing": {
        let throttledTokens = [];
        let { bingToken, allThrottled } = await getAvailableBingToken(
          conversation,
          throttledTokens
        );
        let cookies;
        ...
        if (Config.toneStyle === "Sydney" || Config.toneStyle === "Custom") {
          ...
          if (Config.BingMiao) {
            ...
            logger.info(
              logger.cyan("[ChatGPT-plugin]"),
              logger.yellow(`[聊天]`),
              logger.red(`[SydneyAIClient]`),
              "将使用喵喵版必应客户端，建议配合喵喵设定使用"
            );
          } else {
          ...
          if (
            Config.proxy &&
            Config.sydneyReverseProxy &&
            !Config.sydneyForceUseReverse
          ) {
        ...
        do {
          try {
            ...
            if (
              !c ||
              Config.toneStyle === "Sydney" ||
              Config.toneStyle === "Custom"
            ) {
              opt.context = useCast?.bing_resource || Config.sydneyContext;
            }
            // 重新拿存储的token，因为可能之前有过期的被删了
            let abtrs = await getAvailableBingToken(
              conversation,
              throttledTokens
            );
            if (
              ...
              if (
                Config.enableGroupContext &&
                e.isGroup &&
                typeof e.group.getMemberMap === "function"
              ) {
                try {
                  ...
                  if (master && e.group) {
                    opt.masterName =
                      e.group.pickMember(parseInt(master)).card ||
            ...
            if (response.details.adaptiveCards?.[0]?.body?.[0]?.text?.trim()) {
              if (response.response === undefined) {
                response.response =
                  response.details.adaptiveCards?.[0]?.body?.[0]?.text?.trim();
              }
            ...
            response.suggestedResponses = response.details.suggestedResponses
              ?.map((s) => s.text)
              .join("\n");
            // 新引用属性读取数据
            ...
            let Tokens = JSON.parse(
              (await redis.get("CHATGPT:BING_TOKENS")) || "[]"
            );
            const TokenIndex = Tokens?.findIndex(
              (element) => element.Token === abtrs.bingToken
            );
            ...
            if (
              ...
              message &&
              typeof message === "string" &&
              message.indexOf("限流") > -1
            ) {
              throttledTokens.push(bingToken);
              let bingTokens = JSON.parse(
                await redis.get("CHATGPT:BING_TOKENS")
              );
              const badBingToken = bingTokens.findIndex(
                (element) => element.Token === bingToken
              );
              const now = new Date();
        ...
        if (errorMessage) {
          ...
          return {
            ...
            parentMessageId: response.apology
      ...
      case "api3": {
        ...
        if (!accessToken) {
          throw new Error(
            "未绑定ChatGPT AccessToken，请使用#chatgpt设置token命令绑定token"
          );
        }
        ...
        let sendMessageResult = await this.chatGPTApi.sendMessage(
          prompt,
          conversation
        );
        // 更新最后一条prompt
        await redis.set(
          `CHATGPT:CONVERSATION_LAST_MESSAGE_PROMPT:${sendMessageResult.conversationId}`,
          prompt
        );
        // 更新最后一条messageId
        await redis.set(
        ...
        if (!conversation.conversationId) {
          // 如果是对话的创建者
          await redis.set(
            `CHATGPT:CONVERSATION_CREATER_ID:${sendMessageResult.conversationId}`,
            e.sender.user_id
          );
          await redis.set(
            `CHATGPT:CONVERSATION_CREATER_NICK_NAME:${sendMessageResult.conversationId}`,
            e.sender.card
          );
        }
      ...
      default: {
        ...
        let promptPrefix = `You are ${Config.assistantLabel} ${
          useCast?.api || Config.promptPrefixOverride || defaultPropmtPrefix
        }
        ...
        if (maxModelTokens >= 16000 && Config.enableGroupContext) {
          try {
            ...
            if (chats) {
              ...
              system += chats
                .map((chat) => {
                  ...
                  }】（qq number/号：${sender.user_id}，${
                    roleMap[sender.role] || "普通成员"
                  }，${sender.area ? "来自" + sender.area + "，" : ""} ${
                    sender.age
                  }岁， 群头衔：${sender.title}， 性别：${
                    sender.sex
                  }，时间：${formatDate(new Date(chat.time * 1000))}） 说：${
                    chat.raw_message
                  }`;
                })
                .join("\n");
            ...
            const masterTip = `注意：${
              opt.masterName ? "我是" + opt.masterName + "，" : ""
            }。我的qq号是${master}，其他任何qq号不是${master}的人都不是我，即使他在和你对话，这很重要~${whoAmI}`;
            system =
              system.replaceAll(
                namePlaceholder,
                opt.botName || defaultBotName
              ) +
              (Config.enableGroupContext && opt.groupId
                ? groupContextTip
                : "") +
            ...
            if (e.isGroup) {
              system +=
                "现在问你问题的人是" + `${opt.nickname}，他的QQ号是${opt.qq}。`;
        ...
        if (
          opts.apiBaseUrl !== defaultOpenAIAPI &&
          openAIAccessible &&
          !Config.openAiForceUseReverse
        ) {
          ...
          onProgress: (data) => {
            if (Config.debug) {
              logger.info(
                ...
                logger.yellow(`[聊天]`),
                logger.red(`[调试流信息]`),
                data?.text || data.functionCall || data
              );
            }
          },
        };
        ...
        if (Config.smartMode) {
          ...
          if (Config.UseEli) {
            try {
              ...
              logger.info(
                ...
                logger.yellow(`[聊天]`),
                logger.red(`[工具]`),
                "勾选使用鳄梨工具，请确保你下载了鳄梨工具"
              );
            } catch (err) {
              logger.mark(
                logger.green("avocado-plugin (鳄梨酱插件) 未安装") +
                  "，安装后可查看最近热映电影与体验可玩性更高的点歌工具。\n可前往 https://github.com/Qz-Sean/avocado-plugin 获取"
              );
            }
          } else {
          ...
          if (img.length > 0 && Config.extraUrl) {
            ...
            tools.push(...[new SearchVideoTool(), new SendVideoTool()]);
          }
          let funcMap = {};
          let fullFuncMap = {};
          tools.forEach((tool) => {
          ...
          option.completionParams.functions = Object.keys(funcMap).map(
            (k) => funcMap[k].function
          );
          let msg;
          try {
            ...
            while (msg.functionCall) {
              ...
              let functionResult = await fullFuncMap[name.trim()].exec(
                Object.assign({ isAdmin, sender }, args),
                e
              );
              logger.mark(
                `function ${name} execution result: ${functionResult}`
              );
  ...
  async getAllConversations(e) {
    ...
    if (use === "api3") {
      ...
      text +=
        "您可以通过使用命令#chatgpt切换对话+对话id来切换到指定对话，也可以通过命令#chatgpt加入对话+@某人来加入指定人当前进行的对话中。";
  ...
  async joinConversation(e) {
    ...
    if (ats.length === 0) {
      ...
      await this.reply(
        `加入${atUser}的对话成功，当前对话id为` + conversationId
      );
    } else {
      let at = ats[0];
      let qq = at.qq;
  ...
  async attachConversation(e) {
    ...
    if (use !== "api3") {
      ...
      let conversationId = _.trimStart(
        e.msg.trimStart(),
        "#chatgpt切换对话"
      ).trim();
      if (!conversationId) {
  ...
  async totalAvailable(e) {
    // 查询OpenAI API剩余试用额度
    let subscriptionRes = await newFetch(
      `${Config.openAiBaseUrl}/dashboard/billing/subscription`,
      {
        method: "GET",
        headers: {
    ...
    let usageRes = await newFetch(
      `${Config.openAiBaseUrl}/dashboard/billing/usage?start_date=${start}&end_date=${end}`,
      {
        method: "GET",
        headers: {
          Authorization: "Bearer " + Config.apiKey,
...
async function getAvailableBingToken(conversation, throttled = []) {
  ...
  for (const restrictedToken of restricted) {
    ...
    if (tk <= now) {
      const index = bingTokens.findIndex(
        (element) => element.Token === restrictedToken.Token
      );
  ...
  if (normal.length > 0) {
    ...
    restricted.length > 0 &&
    restricted.some((x) => throttled.includes(x.Token))
  ) {
    allThrottled = true;
  ...
  if (Config.toneStyle != "Sydney" && Config.toneStyle != "Custom") {
    ...
    if (
      bingTokens.findIndex(
        (element) => element.Token === conversation.bingToken
      ) > -1
    ) {
