...
export async function generateVitsAudio(
  text,
  speaker = "随机",
  language = "中日混合（中文用[ZH][ZH]包裹起来，日文用[JA][JA]包裹起来）",
  noiseScale = parseFloat(Config.noiseScale),
  noiseScaleW = parseFloat(Config.noiseScaleW),
  lengthScale = parseFloat(Config.lengthScale)
) {
  ...
  if (space.endsWith("/api/generate")) {
    ...
    logger.warn(
      ...
      logger.yellow(`[语音]`),
      logger.red(`[Vits语音]`),
      `vits api 当前为${space}，已校正为${trimmedSpace}`
    );
    space = trimmedSpace;
  }
  if (space.endsWith("/")) {
  ...
  if (Config.huggingFaceReverseProxy) {
    url = `${Config.huggingFaceReverseProxy}/api/generate?space=${_.trimStart(
      space,
      "https://"
    )}`;
  }
  ...
  try {
    ...
    if (Config.huggingFaceReverseProxy) {
      if (Config.debug) {
        logger.info(
          ...
          logger.yellow(`[语音]`),
          logger.red(`[Vits语音]`),
          "使用huggingface加速反代下载生成音频" + Config.huggingFaceReverseProxy
        );
      }
    ...
    logger.error(
      "生成语音api发生错误，请检查是否配置了正确的api，且仓库是否开放为public",
      response.status
    );
