...
export async function pTimeout(promise, options) {
  const {
    milliseconds,
    fallback,
    message,
...
export async function renderUrl(e, url, renderCfg = {}) {
  ...
  if (Config.cloudRender) {
    url = url.replace(
      `127.0.0.1:${Config.serverPort || 3321}`,
      Config.serverHost || `${await getPublicIP()}:${Config.serverPort || 3321}`
    );
...
export function getAzureRoleList() {
  return azureRoleList
    .map(
      (item) =>
        item.roleInfo +
        (item?.emotion
          ? "-> 支持：" + Object.keys(item.emotion).join("，") + " 情绪。"
          : "")
    )
    .join("\n\n");
}
...
export async function getVitsRoleList(e) {
  const [firstHalf, secondHalf] = [
    vitsRoleList.slice(0, Math.floor(vitsRoleList.length / 2)).join("、"),
    vitsRoleList.slice(Math.floor(vitsRoleList.length / 2)).join("、"),
  ];
  const [chunk1, chunk2] = [
    firstHalf.match(/[^、]+(?:、[^、]+){0,30}/g),
    secondHalf.match(/[^、]+(?:、[^、]+){0,30}/g),
  ];
  const list = [
    await makeForwardMsg(e, chunk1, "vits角色列表1"),
    await makeForwardMsg(e, chunk2, "vits角色列表2"),
  ];
  ...
  try {
    if (role !== "随机") {
      ...
      if (azureRoleList.find((s) => s.code === role.trim())) {
        ...
        if (!speaker) {
          logger.warn(
            "找不到名为" + role + "的发言人,将使用默认发言人 晓晓 发送音频."
          );
      ...
      let languagePrefix = azureRoleList
        .find((config) => config.code === speaker)
        .languageDetail.charAt(0);
      ...
      speaker = role.code;
      languagePrefix = role.languageDetail.charAt(0).startsWith("E")
        ? "英"
        : role.languageDetail.charAt(0);
      pendingText = (await translate(pendingText, languagePrefix)).replace(
        "\n",
        ""
      );
