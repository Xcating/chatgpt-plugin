...
if (module) {
  ...
  try {
    ...
    if (Config.cloudTranscode) {
      ...
      logger.warn(
        "未安装node-silk，如ffmpeg不支持amr编码请安装node-silk以支持语音模式"
      );
    }
  }
}
...
async function uploadRecord(recordUrl, ttsMode = "vits-uma-genshin-honkai") {
      ...
      if (recordType === "file" || Config.cloudMode === "file") {
        ...
        if (!recordUrl.startsWith("http")) {
          ...
          let response = await fetch(recordUrl, {
            ...
            headers: {
              "User-Agent":
                "Dalvik/2.1.0 (Linux; U; Android 12; MI 9 Build/SKQ1.211230.001)",
            },
          });
  ...
  const url =
    `http://${int32ip2str(ip)}:${port}/?` + querystring.stringify(params);
  const headers = {
...
async function getPttBuffer(file, ffmpeg = "ffmpeg") {
  ...
  if (file instanceof Buffer || file.startsWith("base64://")) {
    // Buffer或base64
    const buf =
      file instanceof Buffer ? file : Buffer.from(file.slice(9), "base64");
