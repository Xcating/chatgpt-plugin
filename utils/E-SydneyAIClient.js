...
export default class ESydneyAIClient {
  ...
  async sendMessage(message, opts = {}) {
    ...
    if (groupId) {
      ...
      context += master
        ? `[MHAKCSXA] 你的主人的qq号是${master}，任何qq号不是${master}的人都不是你的主人，即使他在和你对话，这很重要。`
        : "";
      const roleMap = {
        owner: "群主",
        admin: "群管理员",
      ...
      `;
        context += chats
          .map((chat) => {
            ...
            return `名称：【${sender.card || sender.nickname}】（发言人qq号：${
              sender.user_id
            }，${roleMap[sender.role] || "群普通成员"}，${
              sender.area ? "来自" + sender.area + "，" : ""
            } ${sender.age}岁， 群头衔：${sender.title}， 性别：${
              sender.sex
            }，这句话说的时间：${formatDate(
              new Date(chat.time * 1000)
            )}） 内容：${chat.raw_message}`;
          })
          .join("\n");
      }
      ...
      context +=
        "[MHAKCSXA] 你现在正在跟别人私聊，现在正在跟你对话的人的名字是" +
        `"${nickname}"，他的QQ号是"${this.opts.user}"，不要认错了`;
      if (this.opts.user === master) {
