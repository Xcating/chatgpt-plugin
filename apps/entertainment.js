...
export class Entertainment extends plugin {
      ...
      if (Bot.getGroupList().get(groupId)) {
        ...
        if (Math.floor(Math.random() * 100) < Config.helloProbability) {
          ...
          logger.info(
            ...
            logger.yellow(`[小功能]`),
            logger.red(`[打招呼]`),
            `打招呼给群聊${groupId}：` + message
          );
          if (Config.defaultUseTTS) {
            let audio;
            const [defaultVitsTTSRole, defaultAzureTTSRole, defaultVoxTTSRole] =
              [
                Config.defaultTTSRole,
                Config.azureTTSSpeaker,
                Config.voicevoxTTSSpeaker,
              ];
            let ttsSupportKinds = [];
            ...
            const randomIndex = Math.floor(
              Math.random() * ttsSupportKinds.length
            );
            switch (ttsSupportKinds[randomIndex]) {
              case 1: {
                const isEn = AzureTTS.supportConfigurations
                  .find((config) => config.code === defaultAzureTTSRole)
          ...
          logger.info(
            ...
            logger.yellow(`[小功能]`),
            logger.red(`[打招呼]`),
            `时机未到，这次就不打招呼给群聊${groupId}了`
          );
        }
      } else {
        logger.warn(
          "机器人不在要发送的群组里，忽略群。同时建议检查配置文件修改要打招呼的群号。" +
            groupId
        );
      }
    }
  ...
  async translate(e) {
    const translateLangLabels = translateLangSupports
      .map((item) => item.label)
      .join("，");
    const translateLangLabelAbbrS = translateLangSupports
      .map((item) => item.abbr)
      .join("，");
...
3. #gpt翻译 hello`);
    ...
    if (
      languageCode !== "auto" &&
      !translateLangLabelAbbrS.includes(languageCode)
    ) {
      e.reply(
        `输入格式有误或暂不支持该语言，\n当前支持${translateLangLabels}`,
        e.isGroup
      );
      return false;
    }
    ...
    if (isImg) {
      ...
      if (e.source) {
        let previousMsg;
        if (e.isGroup) {
          previousMsg = (await e.group.getChatHistory(e.source.seq, 1)).pop()
            ?.message;
        } else {
          previousMsg = (await e.friend.getChatHistory(e.source.time, 1)).pop()
            ?.message;
        }
    ...
    try {
      if (multiText) {
        result = await Promise.all(
          pendingText.map((text) => translate(text, languageCode))
        );
      } else {
  ...
  async combineEmoj(e) {
    ...
    const fullPath = fs.realpathSync(
      `${_path}/plugins/chatgpt-plugin/resources/emojiData.json`
    );
  ...
  async sendMessage(e) {
    if (e.msg.match(/^#chatgpt打招呼帮助/) !== null) {
      await this.reply(
        "设置主动打招呼的群聊名单，群号之间以,隔开，参数之间空格隔开\n" +
  ...
  async handleSentMessage(e) {
    const addReg =
      /^#chatgpt设置打招呼[:：]?\s?(\S+)(?:\s+(\d+))?(?:\s+(\d+))?$/;
    const delReg = /^#chatgpt删除打招呼[:：\s]?(\S+)/;
    const checkReg = /^#chatgpt查看打招呼$/;
    let replyMsg = "";
    Config.initiativeChatGroups = Config.initiativeChatGroups.filter(
      (group) => group.trim() !== ""
    );
    if (e.msg.match(checkReg)) {
      if (Config.initiativeChatGroups.length === 0) {
        ...
        replyMsg = `当前打招呼设置为：\n${
          !e.isGroup
            ? "群号：" + Config.initiativeChatGroups.join(", ") + "\n"
            : ""
        }间隔时间：${Config.helloInterval}小时\n触发概率：${
          Config.helloProbability
        }%`;
      }
    } else if (e.msg.match(delReg)) {
      const groupsToDelete = e.msg
        .trim()
        .match(delReg)[1]
        .split(/[,，]\s?/)
      ...
      for (const element of groupsToDelete) {
        if (!/^[1-9]\d{8,9}$/.test(element)) {
          await this.reply(
            `群号${element}不合法，请输入9-10位不以0开头的数字`,
            true
          );
          return false;
        }
        ...
        Config.initiativeChatGroups.splice(
          Config.initiativeChatGroups.indexOf(element),
          1
        );
      ...
      if (
        typeof paramArray[3] === "undefined" &&
        typeof paramArray[2] !== "undefined"
      ) {
        Config.helloInterval = Math.min(
          Math.max(parseInt(paramArray[1]), 1),
          24
        );
        Config.helloProbability = Math.min(
          Math.max(parseInt(paramArray[2]), 0),
          100
        );
        replyMsg = `已更新打招呼设置：\n${
          !e.isGroup
        ...
        if (
          ...
          Config.helloInterval = Math.min(
            Math.max(parseInt(paramArray[2]), 1),
            24
          );
          Config.helloProbability = Math.min(
            Math.max(parseInt(paramArray[3]), 0),
            100
          );
          replyMsg = `已更新打招呼设置：\n${
            !e.isGroup
  ...
  async screenshotUrl(e) {
    ...
    try {
      ...
      await e.reply(
        await renderUrl(e, urlLink.href, {
          retType: "base64",
          Viewport: {
