...
export async function getConversations(qq = "", fetchFn = fetch) {
  ...
  let response = await fetchFn(
    `${Config.apiBaseUrl}/conversations?offset=0&limit=20`,
    {
      method: "GET",
      headers: {
  ...
  let result = conversations.items?.sort(
  ...
  for (let i = 0; i < conversations.items.length; i++) {
    ...
    let cachedConversationLastMessage = await redis.get(
      `CHATGPT:CONVERSATION_LAST_MESSAGE_PROMPT:${item.id}`
    );
    if (cachedConversationLastMessage) {
      map[item.id] = cachedConversationLastMessage;
    } else {
      // 缓存中没有，就去查官方api
      let conversationDetailResponse = await fetchFn(
        `${Config.apiBaseUrl}/conversation/${item.id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (Config.debug) {
        logger.mark(
          "conversation detail for conversation " + item.id,
          conversationDetail
        );
      }
      try {
      ...
      let messagesAssistant = messages
        .filter((messages) => messages.author.role === "assistant")
        .sort((a, b) => b.create_time - a.create_time);
      let messagesUser = messages
        .filter((messages) => messages.author.role === "user")
        .sort((a, b) => b.create_time - a.create_time);
      await redis.set(
        `CHATGPT:CONVERSATION_LENGTH:${item.id}`,
      ...
      if (messagesAssistant.length > 0) {
        await redis.set(
          `CHATGPT:CONVERSATION_LAST_MESSAGE_ID:${item.id}`,
          messagesAssistant[0].id
        );
      }
      await redis.set(
        `CHATGPT:CONVERSATION_CREATE_TIME:${item.id}`,
        new Date(conversationDetail.create_time * 1000).toLocaleString()
      );
    }
  }
  let res = [];
  ...
  let promisesPostProcess = result
    ...
    .map(async (conversation) => {
      ...
      conversation.create_time = new Date(
        conversation.create_time
      ).toLocaleString();
      // 这里的时间格式还可以。不用管了。conversation.create_time =
      // title 全是 New chat，不要了
      delete conversation.title;
      conversation.creater = await redis.get(
        `CHATGPT:CONVERSATION_CREATER_NICK_NAME:${conversation.id}`
      );
      ...
      if (conversation.lastPrompt?.length > 80) {
        conversation.lastPrompt =
          conversation.lastPrompt.slice(0, 80) + "......";
      }
...
export async function getLatestMessageIdByConversationId(
  conversationId,
  fetchFn = fetch
) {
  let accessToken = await redis.get("CHATGPT:TOKEN");
  ...
  let conversationDetailResponse = await fetchFn(
    `${Config.apiBaseUrl}/conversation/${conversationId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  if (Config.debug) {
    logger.mark(
      "conversation detail for conversation " + conversationId,
      conversationDetail
    );
  }
  conversationDetail = JSON.parse(conversationDetail).body;
