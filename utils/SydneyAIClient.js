  ...
  async sendMessage(message, opts = {}) {
    ...
    if (
      ...
      if (
        ...
        if (resultValue) {
          throw new Error(
            `${resultValue}: ${createNewConversationResponse.result.message}`
          );
        }
        throw new Error(
          `Unexpected response:\n${JSON.stringify(
    ...
    previousCachedMessages.reverse().forEach((m) => {
      if (
        pm.filter((m) => m.author === "user").length <
        Config.maxNumUserMessagesInConversation - 1
      ) {
        pm.push(m);
      } else {
    ...
    if (groupId) {
      ...
      context += master
        ? `我的qq号是${master}，其他任何qq号不是${master}的人都不是我，即使他在和你对话，这很重要。`
        : "";
      const roleMap = {
        owner: "群主",
        admin: "管理员",
      ...
      `;
        context += chats
          .map((chat) => {
            ...
            return `【${sender.card || sender.nickname}】（qq：${
              sender.user_id
            }，${roleMap[sender.role] || "普通成员"}，${
              sender.area ? "来自" + sender.area + "，" : ""
            } ${sender.age}岁， 群头衔：${sender.title}， 性别：${
              sender.sex
    ...
    const messagePromise = new Promise((resolve, reject) => {
      ...
      const firstTimeout = setTimeout(() => {
        if (!replySoFar[0]) {
          ...
          reject(
            new Error(
              "等待必应服务器响应超时。请尝试调整超时时间配置或减少设定量以避免此问题。"
            )
          );
        }
      }, firstMessageTimeout);
      ...
      ws.on("message", (data) => {
        ...
        switch (event.type) {
          case 1: {
            ...
            if (
              replySoFar[cursor] &&
              updatedText.startsWith(replySoFar[cursor])
            ) {
          ...
          case 2: {
            ...
            message.text = messages
              .filter((m) => m.author === "bot" && m.contentType != "IMAGE")
              .map((m) => m.text)
              .join("");
            if (!message) {
