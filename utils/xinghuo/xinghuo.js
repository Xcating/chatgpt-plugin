...
const createChatUrl = atob(
  "aHR0cHM6Ly94aW5naHVvLnhmeXVuLmNuL2lmbHlncHQvdS9jaGF0LWxpc3QvdjEvY3JlYXRlLWNoYXQtbGlzdA=="
);
const chatUrl = atob(
  "aHR0cHM6Ly94aW5naHVvLnhmeXVuLmNuL2lmbHlncHQtY2hhdC91L2NoYXRfbWVzc2FnZS9jaGF0"
);
let FormData;
try {
...
export default class XinghuoClient {
  ...
  apiErrorInfo(code) {
    switch (code) {
      case 10000:
        return "升级为ws出现错误";
      case 10001:
        return "通过ws读取用户的消息出错";
      case 10002:
        return "通过ws向用户发送消息 错";
      case 10003:
        return "用户的消息格式有错误";
      case 10004:
        return "用户数据的schema错误";
      case 10005:
        return "用户参数值有错误";
      case 10006:
        return "用户并发错误：当前用户已连接，同一用户不能多处同时连接。";
      case 10007:
        return "用户流量受限：服务正在处理用户当前的问题，需等待处理完成后再发送新的请求。（必须要等大模型完全回复之后，才能发送下一个问题）";
      case 10008:
        return "服务容量不足，联系工作人员";
      case 10009:
        return "和引擎建立连接失败";
      case 10010:
        return "接收引擎数据的错误";
      ...
      case 10012:
        return "引擎内部错误";
      case 10013:
        return "输入内容审核不通过，涉嫌违规，请重新调整输入内容";
      case 10014:
        return "输出内容涉及敏感信息，审核不通过，后续结果无法展示给用户";
      case 10015:
        return "appid在黑名单中";
      case 10016:
        return "appid授权类的错误。比如：未开通此功能，未开通对应版本，token不足，并发超过授权 等等";
      case 10017:
        return "清除历史失败";
      case 10019:
        return "表示本次会话内容有涉及违规信息的倾向；建议开发者收到此错误码后给用户一个输入涉及违规的提示";
      case 10110:
        return "服务忙，请稍后再试";
      case 10163:
        return "请求引擎的参数异常 引擎的schema 检查不通过";
      case 10222:
        return "引擎网络异常";
      case 10907:
        return "token数量超过上限。对话历史+问题的字数太多，需要精简输入";
      case 11200:
        return "授权错误：该appId没有相关功能的授权 或者 业务量超过限制";
      case 11201:
        return "授权错误：日流控超限。超过当日最大访问量的限制";
      case 11202:
        return "授权错误：秒级流控超限。秒级并发超过授权路数限制";
      case 11203:
        return "授权错误：并发流控超限。并发路数超过授权路数限制";
      default:
        return "无效错误代码";
    }
  }
  ...
  async getWsUrl() {
    ...
    const url = `wss://spark-api.xf-yun.com${APILink}?${Object.keys(v)
      .map((key) => `${key}=${v[key]}`)
      .join("&")}`;
    return url;
  }
  ...
  async uploadImage(url) {
    ...
    if (respOss.ok) {
      ...
      // 上传接口
      const sparkdeskUrl = `${ossData.data.url}&authorization=${Buffer.from(
        ossData.data.authorization
      ).toString("base64")}&date=${ossData.data.date}&host=${
        ossData.data.host
      }`;
      const respSparkdes = await fetch(sparkdeskUrl, {
        ...
        headers: {
          Cookie: "ssoSessionId=" + this.ssoSessionId + ";",
          authorization: Buffer.from(ossData.data.authorization).toString(
            "base64"
          ),
        },
        body: buffer,
  ...
  async apiMessage(prompt, chatId, ePrompt = []) {
    if (!chatId)
      chatId = (Math.floor(Math.random() * 1000000) + 100000).toString();
      ...
      socket.on("message", async (message) => {
        try {
          ...
          if (messageData.header.code != 0) {
            reject(
              `接口发生错误：Error Code ${
                messageData.header.code
              } ,${this.apiErrorInfo(messageData.header.code)}`
            );
          }
          if (
            messageData.header.status == 0 ||
            messageData.header.status == 1
          ) {
          ...
          if (messageData.header.status == 2) {
            ...
            if (
              messageData.payload.usage.text.total_tokens >= Config.xhMaxTokens
            ) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let option = {
        ...
        headers: Object.assign(this.headers, {
          ...
          "Content-Type":
            "multipart/form-data; boundary=----WebKitFormBoundarycATE2QFHDn9ffeWF",
        }),
        // body: formData,
  ...
  async sendMessage(prompt, chatId, image) {
    ...
    if (
      ...
      if (!Config.xhAppId || !Config.xhAPISecret || !Config.xhAPIKey)
      ...
      if (Config.xhPromptSerialize) {
        ...
        Prompt = Config.xhPrompt
          ? [{ role: "user", content: Config.xhPrompt }]
          : [];
      }
      let response = await this.apiMessage(prompt, chatId, Prompt);
      if (Config.xhRetRegExp) {
        response = response.replace(
          new RegExp(Config.xhRetRegExp, "g"),
          Config.xhRetReplace
        );
      }
      return {
      ...
      let { response } = await this.webMessage(
